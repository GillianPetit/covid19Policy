...
function showData(data) {
	...
	data = data.map(d => ({
		...
		MANcumulativeCases: +d.MANCumulativeCases,
...
//set ranges
	...
	//Define MAN line	
	let MANvalueLine = d3.line()
		.x(d => xscale(d.days))
		.y(d => yscale(d.MANcumulativeCases))
		.defined(d => !! d.MANcumulativeCases)
	...
	//Add dots for each data point for MAN
	body.append("g").selectAll("dot")
		.data(data)
		.enter()
		.append("circle")
		.attr("fill" , "darkblue")
		.attr("r", function(d) { if (d.MANcumulativeCases === 0) {
			return 0} 
			else { return 5}} )
		.attr("cx" , d => xscale(d.days))
		.attr("cy" , d => yscale(d.MANcumulativeCases) )
		.attr("transform" , "translate(100, 0)")
	...
	//Add label to MAN line
	
		let MANmax = d3.max(data, d => d.MANcumulativeCases)

		let MANmaxDays = d3.max(data, function(d) {if(d.MANcumulativeCases===MANmax){return d.days} } )

		body.append("text")
		.attr("transform", "translate(" + xscale(MANmaxDays + 10)  + "," + yscale(MANmax - 100) + ")")
		.attr("text-anchor", "end")
		.style("fill", "darkblue")
		.text("MAN")
